...
if Server then 
    function ExoWeaponHolder:SetGrenaderWeapons()
    
        
        if self.leftWeaponId ~= Entity.invalidId then
            DestroyEntity(Shared.GetEntity(self.leftWeaponId))
        end
        local leftWeapon = CreateEntity(ExoGrenader.kMapName, Vector(), self:GetTeamNumber())
        ...
        local rightWeapon = CreateEntity(ExoGrenader.kMapName, Vector(), self:GetTeamNumber())
