...
Shine.Hook.SetupClassHook( "AvocaSpectator", "HookOnEntityChange", "OnHookOnEntityChange", "PassivePost" )
        ...
        -- Print("vip is %s", vip:GetClassName())
          ...
          if findfreespace == viporigin then findfreespace = FindEntNear(findfreespace) end
             ...
             local angles = Angles(GetPitchFromVector(dir), GetYawFromVector(dir), 0)
              local timeDuration = client:getNextAngleTime()
              local chance = math.random(1,2)
              if chance == 1 then
                client:SetDesiredCamera(timeDuration, {move = true}, client:GetEyePos(), angles, 0)
                self:NotifyGeneric( client, "(Pan-Move)VIP is %s, location is %s", true, vip:GetClassName(), GetLocationName(client) )
              else
                client:SetDesiredCamera(timeDuration, {follow = true}, client:GetEyePos(), angles, 0)
                self:NotifyGeneric( client, "(Pan-Follow)VIP is %s, location is %s", true, vip:GetClassName(), GetLocationName(client) )
              end
                   --SetDesiredCamera(transitionDuration, mode, position, angles, distance, yOffset, callback)
              client.lastswitch = Shared.GetTime()
              client.nextangle = timeDuration
        else
...
local function behindAndLocked(self, client, vip)
    ...
    //  if findfreespace == viporigin then findfreespace.x = findfreespace.x - 2 return end
        //  client:SetOrigin(findfreespace)
         ...
         self:NotifyGeneric( client, "(Lock)VIP is %s, location is %s", true, vip:GetClassName(), GetLocationName(client) )        
end
...
function Plugin:OnHookOnEntityChange(client)
    self:NotifyGeneric( client, "HookOnEntityChange", true )
end
