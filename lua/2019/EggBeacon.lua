...
if Server then
    ...
    function EggBeacon:GetTotalConstructionTime() --not allowed in siege room
    local value = kEggBeaconBuildTime --ConditionalValue(GetIsInSiege(self), kEggBeaconBuildTime * 2, kEggBeaconBuildTime) --
    return value
    end


function EggBeacon:OnConstructionComplete()
        --if GetIsInSiege(self) then kLifeSpan = 4 end --not allowed in siege
        self:AddTimedCallback(TimeUp, kEggBeaconLifeSpan)  
        self:DoYourBusiness()
        self:AddTimedCallback(EggBeacon.DoYourBusiness, kEggBeaconIntervalDelay)
...
function EggBeacon:DoYourBusiness()
   -- Print("DoYourBusiness")
      ...
      for i = 1, #egg do
       ...
       if distance >=20 then //if not in same room..
           if HasMixin(actualegg, "Obstacle") then  actualegg:RemoveFromMesh()end
           actualegg:SetOrigin(FindFreeSpace(self:GetOrigin(), 2, 8))
           //actualegg:SetHive(self) -- This will break the limit. oops.
    ...
    Print("Count of Eggs is %s", count)
    local spawnpoint = FindFreeSpace(self:GetOrigin(), 2, 8)
